"""
Teleoperation with auto-gripper: SO-100/101 leader arm -> i2RT YAM follower arm.

Auto-gripper rule:
  The follower gripper (YAM joint 7) is forced fully open whenever wrist_flex
  (motor 4, the up/down wrist joint) reaches or passes the "straight" angle
  (--straight-angle, default 0 deg).  The leader gripper lever is ignored while
  this override is active.

  Hysteresis: once triggered, the gripper stays open until wrist_flex drops
  back below (--straight-angle - --hysteresis), default -15 deg.  This prevents
  rapid toggling at the boundary.

  Tip: run normally first and watch the status line to see what angle the leader
  reads when the wrist is "straight", then pass that as --straight-angle.

Usage:
  python teleop_yam_auto_gripper.py --leader-port COM3
  python teleop_yam_auto_gripper.py --leader-port COM3 --straight-angle 0 --hysteresis 15

Joint mapping (SO leader -> YAM follower):
  shoulder_pan   -> joint 1
  shoulder_lift  -> joint 2
  elbow_flex     -> joint 3
  wrist_flex     -> joint 4   <-- monitored for auto-gripper
  wrist_roll     -> joint 6   (skips joint 5 which is held at 0)
  gripper        -> joint 7
...
# ── imports ─────────────────────────────────────────────────────────────────
sys.path.insert(0, str(__file__.replace("teleop_yam_auto_gripper.py", "src")))
...
# ── auto-gripper args ────────────────────────────────────────────────────────
parser.add_argument("--straight-angle", type=float, default=0.0,
                    help="wrist_flex angle (deg) considered 'straight' — gripper auto-opens at or above this (default 0)")
parser.add_argument("--hysteresis", type=float, default=15.0,
                    help="Degrees below --straight-angle before manual control resumes (default 15)")
parser.add_argument("--gripper-open-value", type=float, default=1.0,
                    help="Gripper position commanded when auto-open fires (0=closed, 1=fully open, default 1.0)")
...
# ── auto-gripper state ───────────────────────────────────────────────────────
# Gripper opens when wrist_flex >= STRAIGHT_ANGLE.
# Returns to manual when wrist_flex < STRAIGHT_ANGLE - HYSTERESIS.
STRAIGHT_ANGLE          = args.straight_angle
RELEASE_ANGLE           = args.straight_angle + abs(args.hysteresis)
GRIPPER_AUTO_OPEN_VALUE = args.gripper_open_value

_auto_gripper_active = False   # True while wrist_flex override is holding the gripper open
...
# ── rerun init ───────────────────────────────────────────────────────────────
if USE_RERUN:
    rr.init("teleop_yam_auto_gripper", spawn=True)
    time.sleep(1.0)
...
print(f"\nAuto-gripper: opens when wrist_flex <= {STRAIGHT_ANGLE:.1f} deg (straight), "
      f"releases to leader when wrist_flex > {RELEASE_ANGLE:.1f} deg")
print(f"Teleop running at {args.hz} Hz — Ctrl+C to stop.\n")
print("  Leader joints (deg) -> Follower joints (rad)  [AUTO] = gripper overridden")
...
try:
    while True:
        ...
        # ── auto-gripper logic (hysteresis) ──────────────────────────────────
        # wrist_flex == 0 is "straight"; positive = wrist up, negative = wrist down.
        wrist_flex_deg = float(obs["wrist_flex"])
        if wrist_flex_deg <= STRAIGHT_ANGLE:
            _auto_gripper_active = True
        elif wrist_flex_deg > RELEASE_ANGLE:
            _auto_gripper_active = False
        # between STRAIGHT_ANGLE and RELEASE_ANGLE: hold current state (hysteresis)
        ...
        # Gripper: auto-open overrides leader input when wrist_flex reaches straight angle
        if _auto_gripper_active:
            target[-1] = GRIPPER_AUTO_OPEN_VALUE
        elif yam_cal and "gripper" in yam_cal:
            c = yam_cal["gripper"]
        ...
        # Rate-limit arm joints (gripper is not rate-limited)
        ...
        if USE_RERUN:
            ...
            rr.log("auto_gripper/active", rr.Scalars(float(_auto_gripper_active)))
            rr.log("auto_gripper/wrist_flex_deg", rr.Scalars(wrist_flex_deg))

        # Status line
        auto_tag = " [AUTO]" if _auto_gripper_active else "       "
        leader_str = " ".join(f"{float(obs[j]):+6.1f}" for j in LEADER_JOINTS)
        follower_str = " ".join(f"{np.degrees(v):+6.1f}" for v in cmd[:5])
        print(f"\r  L:[{leader_str}]  F:[{follower_str}]  grip:{cmd[-1]:.2f}{auto_tag}", end="", flush=True)
